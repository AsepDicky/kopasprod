      ...
      <table id="filter-form">
         ...
         <tr>
            ...
            <td>
               ...
               <button class="green btn" id="preview-pdf">Preview</button>
               <button class="green btn" id="preview-excel">Preview Excel</button>
            </td>
         </tr>
      </table>
   </form>
...
/* BEGIN SCRIPT */
  ...
  $("#preview-excel").click(function(e){
    ...
    window.open(site_url+"laporan_to_excel/list_jurnal_kas_gl/"+branch_code+"/"+from_date+"/"+thru_date);
  })
   ...
   $("#browse_gl_account").click(function(){
      $.ajax({
         type: "POST",
         url: site_url+"gl/get_gl_account_by_keyword_palsu",
         dataType: "json",
         data: {keyword:$("#keyword","#dialog_gl_account").val()},
         success: function(response){
             html = '<option value="" branch_id="0000" branch_name="Semua Branch">Semua BANK</option>';
            for ( i = 0 ; i < response.length ; i++ )
            {
   ...
   $("#keyword","#dialog_gl_account").keyup(function(e){
      ...
      if(e.which==13)
      {
         $.ajax({
            ...
            data: {keyword:keyword},
            success: function(response){
               html = '<option value="" gl_account_id="*">Semua BANK</option>';
               for ( i = 0 ; i < response.length ; i++ )
               {
   ...
   $("#filter").click(function(e){
      ...
      $.ajax({
         type: "POST",
         dataType: "json",
         url: site_url+"laporan/get_gl_account_history_palsu",
         data: {
