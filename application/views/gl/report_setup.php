    ...
    <div class="row-fluid">
      ...
      <div class="span6">
       <div class="portlet box green">
        ...
        <div class="portlet-body">
            <form action="<?php echo site_url('gl/do_upload_report_setup'); ?>" method="post" enctype="multipart/form-data" id="FormAddUpload" class="form-horizontal">
                <input type="hidden" name="report_code_v2" id="report_code_v2">
                <div class="control-group">
                  <input type="file" id="userfile" name="userfile"/>
                </div>
                <div class="control-group">
                  <button type="submit" id="upload" class="btn blue"><i class="icon-upload"></i> <span>Proses</span></button>
                </div>
            </form>
            <hr>
            <div class="btn-group pull-right">
         ...
         <div class="portlet box green">
          ...
          <div class="portlet-body">
            ...
            <input type="hidden" name="gl_report_item_id" id="gl_report_item_id">
              ...
              <div class="pull-left">
                <select name="sort_account_code" id="sort_account_code" style="margin-left: 10px">
                  <option value="0">SORT CODE</option>
                  <?php
                  $query = $this->db->query("SELECT DISTINCT substring(account_code from 1 for 3) as account_code FROM mfi_gl_account WHERE account_type IN(1,2,3,4,5) ORDER BY account_code ASC");
                  if($query->num_rows() > 0){
              ...
              <div class="pull-left" style="margin-left: 10px">
                <input type="checkbox" class="group-checkable" data-set=".checkboxes" name="push_update" id="push_update"/>&nbsp;<i>update items</i>
              </div>
            <div class="btn-group pull-right">
            ...
            <table class="table table-striped table-bordered table-hover" id="report_item_member_table">
               <thead>
                  <tr>
                     <th style="width:8px;"><input type="checkbox" class="group-checkable" data-set="#report_item_member_table .checkboxes" /></th>
...
$(function(){
  ...
  var FormAdd = $("#FormAddUpload"), alert_error = $('.alert-error')
      progress = $('.progress'), 
      bar = $('.bar'), 
      percent = $('.percent');
  ...
  $("a#link-show-item","#report_table").live('click',function(){
    ...
    $("#report_code_v2").val(report_code);
  ...
  $("#sort_account_type, #sort_account_code").change(function(){
    dTreload3()
  })
  $("a#link-show-item-member","#report_item_table").live('click',function(){
    ...
    $('#report_item_member_table').dataTable({
       ...
       "fnServerParams": function ( aoData ) {
          ...
          aoData.push( { "name": "sort_account_code", "value": $("#sort_account_code").val() } );
        },
       "aoColumns": [
         null,
         null,
  ...
  $("button#btn_save_changes").click(function(){
    ...
    push_update = $('#push_update:checkbox:checked').length > 0;
    var account_code = [];
    var $i = 0;

    if (push_update == false) {
      push_update = 0;
    } else {
      push_update = 1;
    }
    ...
    $.ajax({
      ...
      dataType: "json",
      data: {account_code:account_code,gl_report_item_id:gl_report_item_id,push_update:push_update},
      async: false,
      error: function(){
        alert("Failed to Conenct into Databases !")
      },
