...
<script type="text/javascript">
  table = $('#create_spb').dataTable({
    "bProcessing": true,
    "bServerSide": true,
    "sAjaxSource": site_url+"rekening_nasabah/datatable_create_spb_kas",
    "fnServerParams": function ( aoData ) {
       // aoData.push( { "status_transfer": "status_transfer", "value": '3' } );//status 3 digunakan untuk mengkondisikan agar data pertama tidak di load
   ...
   // var product_code = $("#product_code").val();
    ...
    if(bValid==true){
      $('#create_spb').dataTable({
        ...
        "sAjaxSource": site_url+"rekening_nasabah/datatable_create_spb_kas",
        "fnServerParams": function ( aoData ) {
             aoData.push( { "status_transfer": "status_transfer", "value": '0' } );
             //aoData.push( { "name": "product_code", "value": $('#product_code').val() } );
         },
        "aLengthMenu": [
            [15, 20, -1],
  ...
  $("#btn_save").click(function(){
    // var no_spb = $("#no_spb").val();
    var tanggal_spb = $("#tanggal_spb").val();
  //  var description = $("#product_nickname").val();
    var trx_gl_detail_id = [];
    var $i = 0;
    $("input#checkbox:checked").each(function(){
      trx_gl_detail_id[$i] = $(this).val();
      $i++;
    });

    console.log(trx_gl_detail_id);

    if(trx_gl_detail_id.length==0){
     App.WarningAlert("Please select some row to save !");
      ...
      if(conf){
        $.ajax({
          type: "POST",
          url: site_url+"rekening_nasabah/do_save_spb_kas",
          dataType: "json",
          data: {
             trx_gl_detail_id:trx_gl_detail_id
            // ,no_spb:no_spb
