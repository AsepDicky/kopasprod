...
<div class="row-fluid">
   <div class="span12">
    ...
    <h3 class="page-title">
      Rekening Nasabah <small>Cetak SPB KAS</small>
    </h3>
    <ul class="breadcrumb">
      <li>
        <i class="icon-home"></i>
...
$("#btn-search").click(function(){
  ...
  var tanggal2 = datepicker_replace($("#tanggal2").val());
    $.ajax({
      url: site_url+"rekening_nasabah/get_spb_by_rage_kas",
      type: "POST",
      dataType: "json",
...
$(function(){
      ...
      $("a#previewXLS").live('click',function(){ 
        var id_spb = $(this).data('id_spb');
        window.open('<?php echo site_url();?>rekening_nasabah/do_cetak_spb_xls_kas/'+id_spb+'/preview');
      });
  
      $("a#lampiranPDF").live('click',function(){  
        var no_spb = $(this).data('no_spb');
        window.open('<?php echo site_url();?>rekening_nasabah/do_cetak_transfer_pencairan_kas/'+no_spb+'/previewPDF');
        // alert(id_spb)
      });

      $("a#lampiranXLS").live('click',function(){  
        var no_spb = $(this).data('no_spb');
        window.open('<?php echo site_url();?>rekening_nasabah/do_cetak_transfer_pencairan_kas/'+no_spb+'/previewXLS');
        // alert(id_spb)
      });
