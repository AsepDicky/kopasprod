...
<div class="row-fluid">
    <div class="span12">
        ...
        <h3 class="page-title">RESTITUSI TWP <small>Upload RESTITUSI TWP</small></h3>
        <ul class="breadcrumb">
            <li>
                <i class="icon-home"></i>
            ...
            <li><a href="#">Upload Restitusi TWP</a></li> 
        </ul>
...
<div class="portlet box red" id="wrapper-table">
   <div class="portlet-title">
      <div class="caption"><i class="icon-upload"></i>Data RESTITUSI TWP</div>
      <div class="tools">
    ...
    App.init(); // initlayout and core plugins
      ...
      $('#save').click(function(){
        ...
        if(conf){
          ...
          $.ajax({
            type: "POST",
            url: site_url+"transaction/do_verifikasi_penarikan_twp",
            dataType: "json",
            data: {trx_id:trx_id,account_cash_code:account_cash_code,no_referensi:no_referensi},
            success: function(response){
              if (response.success==true) {
                  if(response.unprocessed=='0'){
                    message = 'Verifikasi Penarikan TWP SUKSES.';
                  }else{
                    message = 'Verifikasi Penarikan TWP SUKSES. '+response.unprocessed+' NIK tidak dapat diproses';
                  }
      ...
      $("a#link-delete").live('click',function(){
        var trx_id = $(this).attr('trx_id');
          ...
          if(conf){
            $.ajax({
              type: "POST",
              url: site_url+"transaction/do_delete_verifikasi_penarikan_twp",
              dataType: "json",
              data: {trx_id:trx_id},
              success: function(response){
                if (response.success==true) {
                    $.alert({
                        title:'Verifikasi Success',icon:'icon-check',backgroundDismiss:false,
                        content:'Delete Penarikan TWP SUKSES.',
                        confirmButtonClass:'btn green',
                        confirm:function(){
                            dTreload();
                        }
      ...
      $('#twp_table').dataTable({
          "bProcessing": true,
          "bServerSide": true,
          "sAjaxSource": site_url+"transaction/datatable_upload_penarikan_twp",
          "aoColumns": [
            { "bSortable": true }
            ,{ "bSortable": true }
            ,{ "bSortable": true }
     ...
     $("a#link-view").live('click',function(){
        var trx_id = $(this).attr('trx_id');
          ...
          window.open('<?php echo site_url();?>laporan_to_excel/preview_penarikan_twp/'+trx_id+'/preview');
     });
