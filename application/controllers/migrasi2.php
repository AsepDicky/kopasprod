...
class Migrasi2 extends GMN_Controller {
	...
	{
		$data['container'] = 'migrasi2/migrasi';
		$data['title'] = "Migrasi";
		$data['boxcolor'] = "blue";
	...
	{
		$data['container'] = 'migrasi/migrasi2';
		$data['title'] = "Migrasi Hutang";
		$data['boxcolor'] = "blue";
		$data['fileupload'] = true;
		$data['filenameupload'] = 'migrasi2018hutang.xlsx';
		$data['button_save'] = '<button id="save_trx" class="btn red" style="font-size:13px;">Save Pengajuan Hutang</button>';
		$data['savefile'] = 'add_pengajuan_pembiayaan_koptel_hutang';
	...
	{
		$data['container'] = 'migrasi2/migrasi2';
...
public function regishutang()
	{
		...
		$data['fileupload'] = false;
		$data['filenameupload'] = 'migrasi_regisakad2018.xlsx';
		$data['button_save'] = '<button id="save_trx" class="btn green" style="font-size:13px;">Save hutang</button>';
		$data['savefile'] = 'proses_registrasi_akad_pembiayaan_hutang';
				...
				if($i>0){
					...
					if($jenis_margin == '2'){
						...
						if ($tmvalid==true) {
							...
							$angsuran_pokok = "0";
							$angsuran_margin = "0";
							$total_angsuran = in_array($row['C'], array(53, 54, 56)) ? $row['M'] : $res_anuitas['total_angsuran'];
						}
					}else{ // flat
                  	...
                  	$responce['rows'][$idx]['cell'] = array(
						 ...
						 ,isset($data['thp']) ? $data['thp']*40/100 : ""
						 ,$angsuran_pokok
						 ,$angsuran_margin
						 ,$total_angsuran
				...
				if (!empty($account_financing_reg_id)) {
					...
					*/
					...
					$data_insert_to_financing = array(
							...
							,'jtempo_angsuran_last'		=>'2021-10-25'
							,'jtempo_angsuran_next'		=>'2021-11-25'
							,'akad_no'					=>$akad_no
	...
	function proses_saldo_awal()
	{
		...
		for ( $i = 0 ; $i < count($noid) ; $i++ ){
			...
			if($debet[$i] > 0 || $kredit[$i] > 0)
			{
				...
				$created_by 	= $this->session->userdata('user_id');
				$created_date	= date('2021-10-31 H:i:s');
				$description = "migration of system";

				$data = array(
						 'trx_gl_id'		=> $trx_gl_id
						,'trx_date'			=> "2017-12-31"
						,'voucher_date'		=> "2017-12-31"
	...
	public function generate_margin_anuitas($data)
	{
		...
		for ( $i = 1 ; $i <= $jangkawaktu ; $i++ ) {
			...
			*/
			...
			if(strtotime($thn_angsur_new) >= strtotime('2021')){
				$saldo_margin_akhir += $saldo_margin_awal+$angsmargin;
				if($blnthn_angsur_new=='01-2021'){
					$angsuran_pokok = $this->convert_numeric($angspokok);
					$angsuran_margin = $this->convert_numeric($angsmargin);
				}
			}
